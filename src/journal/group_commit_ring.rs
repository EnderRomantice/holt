//! Ring-backed WAL coordinator (feature `wal_ring`) — stage 2b.
//!
//! Drop-in replacement for [`super::group_commit::Journal`]: identical
//! `pub(crate)` API, identical on-disk format + replay. Replaces the
//! per-record `Vec` + single crossbeam channel + single batching worker
//! (the measured concurrent-write bottleneck) with the lock-free
//! [`WalRing`] (concurrent reserve+memcpy+publish) + a single background
//! **flusher** that drains the committed prefix into the unchanged
//! [`WalWriter`].
//!
